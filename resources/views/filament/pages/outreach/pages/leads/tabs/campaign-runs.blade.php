        @include('filament.pages.outreach.pages.leads.filter-header', [
            'show' => "activeNav === 'Leads' && activeTab === 'Lead Campaigns' && ! leadDetailOpen",
            'focusWhen' => "activeTab === 'Lead Campaigns'",
            'inputRef' => 'leadCampaignsOverlayInput',
            'title' => 'Campaign Runs',
            'description' => 'Multi-line campaign run view for reviewing leads faster.',
        ])

        <section data-card-surface x-cloak x-show="activeNav === 'Leads' && activeTab === 'Lead Campaigns' && ! leadDetailOpen" class="mx-6 mb-6 mt-4 overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-gray-900/5">
                        ...
                        <template x-for="(row, rowIndex) in loadingRows()" :key="'lead-campaign-multiline-' + row.campaignName + row.name + row.phone + row.email + row.age">
                            <tr x-on:click="openLeadDetails(row)" class="cursor-pointer transition-colors hover:bg-gray-50 max-sm:grid max-sm:grid-cols-2 max-sm:items-start max-sm:gap-x-4 max-sm:gap-y-4 max-sm:px-4 max-sm:py-4" :class="rowIndex === loadingRows().length - 1 ? '' : 'border-b border-gray-100'">
                                ...
                                <td class="px-4 py-4 align-top max-sm:order-2 max-sm:block max-sm:min-h-[56px] max-sm:p-0">
                                    ...
                                    <div class="mt-1">
                                        <span class="inline-flex h-[26px] max-w-[140px] items-center rounded-full px-2 py-0 text-xs font-medium ring-1 ring-inset" :class="campaignPillClass(row.campaignStatus)">
                                ...
                                <td class="px-4 py-4 align-top max-sm:order-4 max-sm:block max-sm:min-h-[56px] max-sm:p-0">
                                    <div class="flex h-[26px] items-center text-sm font-medium text-gray-900">First</div>
                                    <div class="mt-1">
                                        <span class="inline-flex h-[26px] w-fit max-w-full min-w-0 items-center rounded-full px-2 py-0 text-xs font-medium ring-1 ring-inset" :class="campaignPillClass(row.firstInteraction)">
                                ...
                                <td class="px-4 py-4 align-top max-sm:order-5 max-sm:block max-sm:min-h-[56px] max-sm:p-0">
                                    <div class="flex h-[26px] items-center text-sm font-medium text-gray-900">Follow-Up</div>
                                    <div class="mt-1">
                                        <span class="inline-flex h-[26px] w-fit max-w-full min-w-0 items-center rounded-full px-2 py-0 text-xs font-medium ring-1 ring-inset" :class="campaignPillClass(row.followUp)">
