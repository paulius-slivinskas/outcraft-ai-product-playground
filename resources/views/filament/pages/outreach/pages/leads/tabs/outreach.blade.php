        @include('filament.pages.outreach.pages.leads.filter-header', [
            'show' => "activeNav === 'Leads' && activeTab === 'Outreach' && ! leadDetailOpen",
        ...
        <section data-card-surface x-cloak x-show="activeNav === 'Leads' && activeTab === 'Outreach' && ! leadDetailOpen" class="mx-6 mb-6 mt-4 overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-gray-900/5">
            <div class="overflow-x-auto max-sm:overflow-visible">
                <table class="w-full border-collapse text-sm max-sm:block sm:min-w-[980px] sm:table-fixed">
                    <thead class="max-sm:hidden">
                        <tr class="border-b border-gray-100 bg-gray-50 text-left text-sm font-semibold text-gray-950">
                            <th class="w-[300px] px-6 py-4">Lead</th>
                    ...
                    <tbody class="max-sm:block">
                        ...
                        <template x-for="(row, rowIndex) in loadingRows()" :key="row.name + row.email + row.age + row.result">
                            <tr class="max-sm:grid max-sm:grid-cols-2 max-sm:items-start max-sm:gap-x-4 max-sm:gap-y-4 max-sm:px-4 max-sm:py-4" :class="rowIndex === loadingRows().length - 1 ? '' : 'border-b border-gray-100'">
                                ...
                                <td class="px-4 py-4 align-top max-sm:order-2 max-sm:block max-sm:min-h-[56px] max-sm:p-0">
                                    ...
                                    <div class="mt-1 hidden min-w-0 flex-wrap items-center gap-2 max-sm:flex">
                                        ...
                                        <button type="button" x-show="row.content === 'View'" x-on:click="openLeadDetails(row)" class="inline-flex text-left">
                                            <span class="outcraft-label inline-flex h-[26px] max-w-[76px] cursor-pointer items-center rounded-full bg-gray-50 px-2 py-0 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10">
                                                <span class="truncate">View</span>
                                            </span>
                                        </button>
                                        <span x-show="row.content && row.content !== 'View'" class="inline-flex">
                                            <span class="outcraft-label inline-flex h-[26px] max-w-[76px] cursor-pointer items-center gap-1 rounded-full bg-gray-50 px-2 py-0 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10">
                                ...
                                <td class="px-4 py-4 align-top max-sm:hidden">
                                    <button type="button" x-show="row.content === 'View'" x-on:click="openLeadDetails(row)" class="group relative inline-flex text-left">
                                        <span class="outcraft-label inline-flex h-[26px] max-w-[76px] cursor-pointer items-center rounded-full bg-gray-50 px-2 py-0 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10 transition group-hover:text-gray-950">
                                            <span class="truncate">View</span>
                                        </span>
                                    ...
                                    <span x-show="row.content && row.content !== 'View'" class="group relative inline-flex">
                                        <span class="outcraft-label inline-flex h-[26px] max-w-[76px] cursor-pointer items-center gap-1 rounded-full bg-gray-50 px-2 py-0 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10 transition group-hover:text-gray-950">
                                            <span class="outcraft-icon !text-[18px] !leading-[18px] ">play_circle</span>
                                            <span class="truncate leading-[18px]" x-text="row.content"></span>
                                        </span>
                                        <span class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 -translate-x-1/2 translate-y-1 whitespace-nowrap rounded-lg bg-gray-900 px-3 py-2 text-xs font-medium text-white opacity-0 shadow-sm transition group-hover:translate-y-0 group-hover:opacity-100">
                                ...
                                <td class="px-4 py-4 align-top max-sm:order-4 max-sm:block max-sm:min-h-[56px] max-sm:p-0">
                                    <div class="mb-1 hidden h-[26px] items-center text-sm font-medium text-gray-900 max-sm:flex">Outcome</div>
                                    <span class="group relative inline-flex">
                                        <span class="outcraft-label inline-flex h-[26px] max-w-[138px] items-center rounded-full px-2 py-0 text-xs font-medium ring-1 ring-inset" :class="pillClass(row.outcome)">
                                ...
                                <td class="px-4 py-4 align-top max-sm:order-5 max-sm:block max-sm:min-h-[56px] max-sm:p-0">
                                    <div class="mb-1 hidden h-[26px] items-center text-sm font-medium text-gray-900 max-sm:flex">Result</div>
                                    <span class="group relative inline-flex">
                                        <span class="outcraft-label inline-flex h-[26px] max-w-[98px] items-center rounded-full px-2 py-0 text-xs font-medium ring-1 ring-inset" :class="pillClass(row.result)">
                        ...
                        <tr x-show="! isLoading && filteredRows().length === 0">
                            <td colspan="8" class="px-8 py-16 text-center text-gray-500">No outreach records match these filters.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
